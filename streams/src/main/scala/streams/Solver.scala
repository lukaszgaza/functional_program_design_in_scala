...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    case (block, move) #:: rest =>
      val newExplored = explored + block
      val newNeighbors = newNeighborsOnly(neighborsWithHistory(block, move), explored)
      initial #::: from(rest ++ newNeighbors, newExplored)
  }
  ...
  lazy val solution: List[Move] = pathsToGoal.headOption match {
    ...
    case Some((block, history)) => history.reverse
  }
}